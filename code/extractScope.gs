function extractScope(cvss3) {
  ...
  const match = cvss3.replace("CVSS","").match(regex);
  var translatedValue = "";
  ...
  if (match) {
    ...
    if (extractedString === 'U') {
        translatedValue = 'Unchanged';
      } else if (extractedString === 'C') {
        translatedValue = 'Changed';
      }  else {
